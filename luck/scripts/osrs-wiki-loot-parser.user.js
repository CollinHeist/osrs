...
(() => {
  ...
  function parseFraction(value) {
    const match = value?.replace(/,/g, '').match(/^\s*(\d+(?:\.\d+)?)\s*\/\s*(\d+(?:\.\d+)?)\s*$/)
  ...
  function parseDropRow(row, tableName) {
    ...
    const rate = parseFraction(fraction) ?? (/\balways\b/i.test(rarityText) ? [1, 1] : null)
    if (!rate) {
      return {
  ...
  function parseTable(table, index) {
    const name = precedingHeading(table)
    const warnings = []
    const drops = []

    for (const row of table.querySelectorAll('tbody tr')) {
      const result = parseDropRow(row, name)
  ...
  function tableMarkup(table, tableIndex) {
    ...
    return `
      <details class="osrs-parser-table" data-table-index="${tableIndex}">
        <summary>
          <input type="checkbox" class="osrs-parser-group-enabled" checked>
          <h3>${escapeHtml(table.name)}</h3>
          <span class="osrs-parser-muted">${table.drops.length} parsed</span>
        </summary>
    ...
    overlay.innerHTML = `
      <section id="osrs-loot-parser-panel" role="dialog" aria-modal="true" aria-labelledby="osrs-parser-title">
        ...
        <p class="osrs-parser-muted">
          ...
          Wiki table headings do not encode those mechanics.
        </p>
    ...
    for (const details of overlay.querySelectorAll('.osrs-parser-table')) {
      ...
      if (groupDropIds.length > 0) {
        groups.push({
          ...
          rollsPerUnit: Number(details.querySelector('.osrs-parser-rolls').value) || 1,
          drops: groupDropIds,
        })
      }
    }
